...
local function createDrawing(drawingType)
    ...
    elseif drawingType == "Quad" then
        ...
        local PointA = DrawingLibL.new("Line")
        local PointB = DrawingLibL.new("Line")
        local PointC = DrawingLibL.new("Line")
        local PointD = DrawingLibL.new("Line")

        return setmetatable({}, {
            __tostring = function() return "Drawing" end, -- added for consistency
    ...
    elseif drawingType == "Triangle" then
        ...
        _linePoints.A = DrawingLibL.new("Line")
        _linePoints.B = DrawingLibL.new("Line")
        _linePoints.C = DrawingLibL.new("Line")
        return setmetatable(table.create(0), {
            __tostring = function() return "Drawing" end,
            __newindex = function(_, index, value)
...
function DrawingLibL.new(drawingType)
    local obj = createDrawing(drawingType)
    if obj ~= nil then
        drawingObjects[obj] = true
    end
    return obj
end

getgenv().Drawing = DrawingLibL

getgenv().getrenderproperty = newcclosure(function(tab, prop)
...
getgenv().setrenderproperty = newcclosure(function(tab, prop, val)
    if typeof(tab) == "table" then
        tab[prop] = val;
    end
end) 

getgenv().isrenderobj = newcclosure(function(tab)
    return drawingObjects[tab] == true
end) 
